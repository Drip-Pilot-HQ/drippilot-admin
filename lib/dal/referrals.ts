...
export async function getReferrersSummary(): Promise<ReferrerSummary[]> {
  ...
  return (data ?? []).map((rc: any) => {
    const signups: any[] = rc.referral_signups ?? []
    const allCommissions: CommissionDetail[] = signups.flatMap((s: any) =>
      (s.referral_commissions ?? []).map((c: any) => ({
        id: c.id,
    ...
    return {
      ...
      total_commission_cents: allCommissions.reduce((s, c) => s + c.commission_amount_cents, 0),
      pending_commission_cents: allCommissions.filter(c => c.status === 'pending').reduce((s, c) => s + c.commission_amount_cents, 0),
      paid_commission_cents: allCommissions.filter(c => c.status === 'paid').reduce((s, c) => s + c.commission_amount_cents, 0),
