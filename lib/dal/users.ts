...
export type UserDetail = UserRecord & {
  ...
  memberships: { workspace_id: string; workspace_name: string; role: string; status: string }[]
}
...
export async function getUserDetail(userId: string): Promise<UserDetail | null> {
  ...
  const ownedWorkspaces: any[] = data.workspace ?? []
  const memberships: any[] = data.workspace_membership ?? []
...
export async function getUserStats(): Promise<{
  ...
  const [{ count: total }, { count: recent_7d }, { data: authData }] = await Promise.all([
  ...
  const banned = (authData?.users ?? []).filter((u) => u.banned_until != null).length
