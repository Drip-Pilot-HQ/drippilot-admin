...
export const getCustomPlanByPriceId = cache(async (stripePriceId: string): Promise<CustomPlan | null> => {
...
export const getCustomPlansByWorkspace = cache(async (workspaceId: string): Promise<CustomPlan[]> => {
  const supabase = createAdminClient()
  const { data, error } = await supabase
    .from('custom_plans')
    .select('*')
