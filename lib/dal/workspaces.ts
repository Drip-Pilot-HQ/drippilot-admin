...
export async function getWorkspaceDetail(workspaceId: string): Promise<WorkspaceDetail | null> {
  ...
  const [{ data, error }, countResults] = await Promise.all([
    ...
    Promise.all([
      ...
      supabase.from('email_alias').select('*', { count: 'exact', head: true }).eq('workspace_id', workspaceId),
      supabase.from('phone_alias').select('*', { count: 'exact', head: true }).eq('workspace_id', workspaceId),
    ]),
  ])

  if (error || !data) return null

  const [{ count: campaignCount }, { count: leadCount }, { count: emailCount }, { count: phoneCount }] = countResults

  // PostgREST returns one-to-many embeds as arrays even with unique constraints
  const billing = (data.workspace_billing as any[])?.[0] ?? null
  const limits = (data.workspace_plan_limits as any[])?.[0] ?? null
  const usage = (data.workspace_plan_usage as any[])?.[0] ?? null
  const members: any[] = data.workspace_membership ?? []
  ...
  return {
    ...
    usage: usage
      ? {
          ...
          credits_granted_this_period: Number(usage.credits_granted_this_period),
...
export async function getWorkspaceStats(): Promise<{ total: number; active_subscriptions: number }> {
  await requireAdmin()
  const supabase = createAdminClient()
