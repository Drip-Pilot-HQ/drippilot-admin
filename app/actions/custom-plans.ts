...
export async function createCustomPlanAction(
  ...
  const ownerEmail = (workspace.users as unknown as { email: string } | null)
    ?.email;
  ...
  let stripeCustomerId: string | undefined =
    billing?.stripe_customer_id ?? undefined;
 ...
 */
export async function archiveCustomPlanAction(
  customPlanId: string,
  workspaceId: string,
): Promise<void> {
  const supabase = createAdminClient();
